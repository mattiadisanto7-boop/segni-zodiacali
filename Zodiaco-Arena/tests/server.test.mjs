...
test("l’health check identifica senza ambiguità la versione distribuita",async()=>{
 ...
 assert.equal(response.status,200);assert.equal(health.ok,true);assert.equal(health.version,"6.3.0");assert.equal(health.roundSeconds,.9);assert.equal(health.progressiveRevealSeconds,.25);
 assert.deepEqual(health.features,["guesswho-scelta-privata","lobby-online-persistente","atlante-tema-natale","tessere-totale-360","profilo-264-scene","domande-60-secondi","audio-celeste","costellazioni-rotazione-360"]);
});
...
test("Indovina Chi online fa scegliere segreti privati, alterna i turni e torna nella stessa lobby",async()=>{
 ...
 await request(`/api/rooms/${host.code}/start`,credentials(host));
 const choosing=await until(host,value=>value.status==="guesswho-choosing","scelta identità");assert.equal(choosing.guessWho.mySecret,null);assert.deepEqual(choosing.guessWho.chosenPlayerIds,[]);
 await request(`/api/rooms/${host.code}/guesswho-secret`,credentials(host,{sign:"Leone"}));const hostChosen=await until(host,value=>value.guessWho?.mySecret?.name==="Leone","segreto host");const guestPrivate=await snapshot(guest);assert.equal(guestPrivate.guessWho.mySecret,null);assert.equal(guestPrivate.guessWho.opponentSecret,null);assert.ok(hostChosen.guessWho.chosenPlayerIds.includes(host.playerId));
 await request(`/api/rooms/${host.code}/guesswho-secret`,credentials(guest,{sign:"Acquario"}));const hostView=await until(host,value=>value.status==="guesswho-playing","partita Indovina Chi"),guestView=await until(guest,value=>value.status==="guesswho-playing","partita Indovina Chi guest");
 assert.equal(hostView.question,null);assert.equal(guestView.question,null);assert.equal(hostView.guessWho.opponentSecret,null);assert.equal(guestView.guessWho.opponentSecret,null);assert.equal(hostView.guessWho.mySecret.name,"Leone");assert.equal(guestView.guessWho.mySecret.name,"Acquario");
 ...
 await request(`/api/rooms/${host.code}/return-lobby`,credentials(guest));
 const lobbyHost=await until(host,value=>value.status==="lobby","ritorno host alla lobby"),lobbyGuest=await until(guest,value=>value.status==="lobby","ritorno guest alla lobby");assert.equal(lobbyHost.code,host.code);assert.equal(lobbyGuest.code,host.code);assert.deepEqual(lobbyHost.players.map(player=>player.id).sort(),[host.playerId,guest.playerId].sort());assert.equal(lobbyHost.guessWho,null);
 await request(`/api/rooms/${host.code}/start`,credentials(host));const choosingAgain=await until(host,value=>value.status==="guesswho-choosing"&&value.guessWho?.matchId!==matchId,"nuova partita dalla stessa lobby");assert.equal(choosingAgain.players.length,2);
});
