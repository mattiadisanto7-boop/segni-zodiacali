...
const rooms=new Map(),ROUND_MS=Number(process.env.ZODIAC_ROUND_MS||20000),BET_MS=Number(process.env.ZODIAC_BET_MS||15000),REVEAL_MS=Number(process.env.ZODIAC_REVEAL_MS||4000),TILE_REVEAL_MS=Number(process.env.ZODIAC_TILE_REVEAL_MS||2600),ROOM_TTL=30*60*1000;
...
function tileSnapshot(room,viewerId){
 ...
 return{phase:game.phase,category:game.category,battle:game.battle,activePlayerId:activeId,defenderId,myCount:myDeck.length,opponentCount:opponentDeck.length,myTop:myDeck.slice(0,2),opponentChoices:opponentDeck.slice(0,2).map(publicTile),attackerCard:viewerId===activeId?myDeck[0]:publicTile(game.decks[activeId]?.[0]),lastBattle:game.lastBattle};
}
...
function questionSnapshot(room,question,revealed){
 ...
 const words=visible.prompt.split(/\s+/),elapsed=Math.max(0,Date.now()-room.roundStartedAt),count=Math.max(1,Math.min(words.length,Math.floor(elapsed/ROUND_MS*words.length)+1));
...
function scoreQuestion(room){
 const q=room.questions[room.round],phase=q.phase||"normal",answers=Object.entries(room.answers).map(([id,a])=>({id,...a}));for(const answer of answers)answer.correct=answer.choice===q.answer;
 if(phase==="hot"){
  const correct=answers.filter(a=>a.correct).sort((a,b)=>a.elapsed-b.elapsed);
  correct.forEach((answer,index)=>answer.points=index===0?speedPoints(answer.elapsed,420,280):170);
 }else if(phase==="bet"){
  for(const answer of answers)answer.points=answer.correct?(room.bets[answer.id]||50):-(room.bets[answer.id]||50);
 }else if(phase==="slow"){
  const winner=answers.filter(a=>a.correct).sort((a,b)=>a.elapsed-b.elapsed)[0];if(winner)winner.points=speedPoints(winner.elapsed,520,190);
 }else if(phase==="steal"){
  const winner=answers.filter(a=>a.correct).sort((a,b)=>a.elapsed-b.elapsed)[0];
  if(winner){const loser=room.players.find(player=>player.id!==winner.id),amount=Math.min(Math.max(0,loser.score),speedPoints(winner.elapsed,360,100));winner.points=amount;loser.score-=amount;room.roundOutcome={winnerId:winner.id,stolen:amount}}
 }else{
 ...
 if(phase==="bet")room.roundOutcome={bets:{...room.bets}};
}
...
const server=http.createServer(async(req,res)=>{
 ...
 try{
  if(url.pathname==="/api/health")return json(res,200,{ok:true,version:APP_VERSION,rooms:rooms.size,features:["guesswho-manuale","profilo-vivente","campionato-4-fasi","tessere-cpu-online"]});
